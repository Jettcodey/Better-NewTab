 ...
 * @author Lea Verou
 * Edited for this project by Jettcodey
 */
...
class MarkdownElement extends HTMLElement {
	...
	async render() {
		...
		let markedModule;
		if (!window.marked) {
			markedModule = await import(URLs.marked);
			window.marked = markedModule.default || markedModule;
		}
		...
		let PrismModule;
		if (!window.Prism && URLs.Prism && this.querySelector("code")) {
			PrismModule = await import(URLs.Prism);
	...
	static async sanitize(html) {
		let DOMPurifyModule;
		if (!window.DOMPurify) {
			DOMPurifyModule = await import(URLs.DOMPurify);
...
customElements.define('markdown-span', MarkdownSpan);
